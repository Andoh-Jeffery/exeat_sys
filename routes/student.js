  ...
  try {
    ...
    if (req.session.isAuthorize === "0") {
      const issued = await db
        .collection("exeat")
        .where("hasReturn", "==", false)
        .get();
      const teacherData = await db
        .collection("teacher")
        .where("house", "==", req.session.isAuthorize)
        .get();
      res.status(200).render("viewIssuedExeat", {
        title: "issued exeat",
        issuedExeat: issued,
        moment: moment,
        auth: req.session.isAuthorize,
...
router.put("/issued/:id", isAuth, async (req, res) => {
  ...
  try {
    ...
    const studentData = await db.collection("exeat").doc(id).get();
    const message = `Dear parent, your ward ${
      studentData.data().studentName
    } has reported to school on this day ${moment().format("LL")}.`;
    sms(studentData.data().parentTelephone, message);
    res.status(201).json({ message: "updated to return" });
  } catch (error) {
...
router.post("/add", async (req, res) => {
  try {
    const { firstName, middleName, lastName, house, course, parentTelephone } =
      req.body;
...
router.get("/issue/:id", isAuth, async (req, res) => {
  ...
  try {
    ...
    res.status(200).render("issue_exeat", {
      title: "issue exeat",
      exeat: issue.data(),
      auth: req.session.isAuthorize,
      data: teacherData,
